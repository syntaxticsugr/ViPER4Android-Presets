...
def normalize(rmin, rmax, tmin, tmax, value):
    new_value = (value-rmin)/(rmax-rmin)*(tmax-tmin)+tmin
    return abs(int(new_value))



# Function to convert old format Presets to new format
def convert_presets(old_presets_folder, new_presets_folder, default_id1_preset_file, default_id2_preset_file):
    ...
    for old_preset_file in old_preset_files:
        ...
        # Old format Preset
        # New format Preset
        old_preset_file = '{}/{}'.format(old_presets_folder, old_preset_file)
        new_preset_file = '{}/{}'.format(new_presets_folder, os.path.basename(old_preset_file))
        with open(default_id1_preset_file) as default_id1_preset, open(default_id2_preset_file) as default_id2_preset, open(old_preset_file) as old_preset, open(new_preset_file, 'w') as new_preset:
            ...
            if("speaker" in os.path.basename(old_preset_file).lower()):
                default_preset_content = default_id2_preset.read().splitlines()
            else:
                default_preset_content = default_id1_preset.read().splitlines()
            ...
            for default_line in default_preset_content:
                ...
                # Master Limiter & Playback Gain Control kept same as ViPER defaults
                    ...
                    for old_line in old_preset_content:
                        ...
                        if ("65560" in default_line):
                            if (("65560" in old_line) or ("65598" in old_line)):
                                ...
                                if 10 < int(old_line[3]) : old_line[3] = str(normalize(0, 100, 0, 10, int(old_line[3])))
                        ...
                        if (default_line[default_line.find('"')+1:default_line.find('"', default_line.find('"')+1)] == old_line[old_line.find('"')+1:old_line.find('"', old_line.find('"')+1)]):
                            ...
                            if ("65554;65556" in default_line):
                                ...
                                old_line = old_line.split('"')
                                if 8 < int(old_line[3]) : old_line[3] = str(normalize(120, 200, 0, 8, int(old_line[3])))
                                old_line = '"'.join(old_line)
                            ...
                            if ("65555" in default_line):
                                # Normalizing to range of 0 - 10
                                old_line = old_line.split('"')
                                if 11 < int(old_line[3]) : old_line[3] = str(normalize(100, 200, 0, 10, int(old_line[3])))
                                old_line = '"'.join(old_line)
                                new_preset.write(old_line + "\n")
                                break
                            ...
                            if ("65558" in default_line):
                                ...
                                old_line = old_line.split('"')
                                if 19 < int(old_line[3]) : old_line[3] = str(normalize(100, 2000, 0, 19, int(old_line[3])))
                                old_line = '"'.join(old_line)
                                new_preset.write(old_line + "\n")
                                break                      
                            ...
                            if ("65573" in default_line):
                                ...
                                old_line = old_line.split('"')
                                if 100 < int(old_line[3]) : old_line[3] = str(normalize(100, 2100, 0, 100, int(old_line[3])))
                                old_line = '"'.join(old_line)
                                new_preset.write(old_line + "\n")
                                break   
                            ...
                            if ("65577" in default_line):
                                ...
                                old_line = old_line.split('"')
                                if 11 < int(old_line[3]) : old_line[3] = str(normalize(50, 600, 0, 11, int(old_line[3])))
                                old_line = '"'.join(old_line)
                                new_preset.write(old_line + "\n")
                                break    
                            ...
                            if ("65580" in default_line):
                                ...
                                old_line = old_line.split('"')
                                if 9 < int(old_line[3]) : old_line[3] = str(normalize(0, 450, 0, 9, int(old_line[3])))
                                old_line = '"'.join(old_line)
                                new_preset.write(old_line + "\n")
                                break 
...
default_id1_preset_file = r''
default_id2_preset_file = r''
...
# Function Call
convert_presets(old_presets_folder, new_presets_folder, default_id1_preset_file, default_id2_preset_file)
